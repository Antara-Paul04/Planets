...
export async function runModeration({ image, name, cfg }) {
  ...
  // moderate whatever text we have: extracted text plus the planet's name
  const combinedText = [ocrResult && ocrResult.text, name].filter(Boolean).join('\n').trim();
  let textResult = null;
  if (combinedText) {
    try {
      textResult = await providers.text.moderate(combinedText);
    } catch {
      failures += 1;
    }
  }

  const decision = decide({ image: imageResult, text: textResult, ocr: ocrResult, failures }, cfg);
  ...
  const job = {
    ...
    text_score: textResult ? round3(textResult.score) : null,
    provider: [
      imageResult && imageResult.provider,
      ocrResult && ocrResult.provider,
      textResult && textResult.provider,
    ].filter(Boolean).join('+') || null,
    latency_ms: ms,
    error_category: failures > 0 ? 'provider_failure' : null,
  };
  try {
    await getStore(cfg).saveJob(job);
  } catch {
    // record-keeping must never block a decision
  }
